...
class ImageNet(BaseDataset):
    ...
    def setup(self, stage: str):
        if stage == "fit":
            full_dataset = td.ImageFolder(f'{self.data_dir}/train/', transform=self.train_transforms)
            self.split_train_val(full_dataset)
        elif stage == "test":
            self.test = td.ImageFolder(f'{self.data_dir}/val/', transform=self.test_transforms)
